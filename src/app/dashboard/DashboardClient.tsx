...
import { ProcessingStatus, type Postcard } from '@/types/database';
...
export function DashboardClient({ initialPostcards }: DashboardClientProps) {
  const { user, isLoaded } = useUser();
  const { postcards, loading, error, deletePostcard, fetchPostcards, refreshAllPostcards } =
    usePostcards({ initialData: initialPostcards });
  ...
  // Refresh when the tab regains focus after being hidden for >2s.
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible') {
        ...
        visibilityTimeout = setTimeout(() => {
          if (isLoaded && user && !loading) fetchPostcards(false);
        }, 2000);
      }
    };
    if (isLoaded && user) {
  ...
  }, [isLoaded, user, loading, fetchPostcards]);
  ...
  const filteredPostcards = useMemo(() => {
    if (!Array.isArray(postcards)) return [];
        ...
        <div className="container mx-auto px-4 md:px-6 lg:px-8 py-6 md:py-8">
          <div className="flex items-center justify-center min-h-[400px]">
            <div className="text-center">
              ...
              <Button onClick={() => fetchPostcards(false)}>Intentar de nuevo</Button>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="container mx-auto px-4 md:px-6 lg:px-8 py-6 md:py-8 lg:py-10">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            ...
            <Link href="/dashboard/new">
              <Button
                size="lg"
                className="w-full sm:w-auto bg-linear-to-r from-primary to-ring hover:from-primary/90 hover:to-ring/90 text-white shadow-md hover:shadow-lg transition-all"
              >
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <Select
                      value={statusFilter}
                      onValueChange={(value) => setStatusFilter(value as ProcessingStatus | 'all')}
                    >
            ...
            <div className="flex items-center justify-center min-h-[400px]">
              <div className="text-center max-w-md">
                ...
                <p className="text-muted-foreground mb-6">
                  Crea tu primera postal de realidad aumentada subiendo una foto y video. Comparte experiencias mágicas que cobran vida a través de cualquier cámara.
                </p>
                <Link href="/dashboard/new">
                  <Button
                    size="lg"
                    className="bg-linear-to-r from-primary to-ring hover:from-primary/90 hover:to-ring/90 text-white shadow-md"
                  >
                    <Plus className="mr-2 h-5 w-5" />
