...
// Visual layers shared by every pill. Kept identical to the original markup so
// the look/animation is unchanged; only the DOM nesting moved (see below).
...
// Base pill (the <li> itself). `expand` controls the active/hover width.
const pillClass = (expandClass: string) =>
  `relative w-[38px] h-[38px] sm:w-[44px] sm:h-[44px] bg-card/80 backdrop-blur-sm shadow-sm rounded-full flex items-center justify-center transition-all duration-500 hover:shadow-none group cursor-pointer ${expandClass}`;

// The interactive surface filling the <li>, so <a>/<button> live INSIDE the
// <li> (valid list markup) instead of wrapping it.
const surfaceClass = 'absolute inset-0 flex items-center justify-center rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring';
        ...
        <ul className="flex gap-2 sm:gap-3">
          {menuItems.map(({ title, icon, href, gradientFrom, gradientTo, requiresAuth }, idx) => {
            ...
            const item = (
              <li
                ...
                className={pillClass(isActive ? 'w-[90px] sm:w-[102px] shadow-none' : 'hover:w-[90px] sm:hover:w-[102px]')}
              >
                <Link href={href} aria-label={title} className={surfaceClass}>
                  <PillContents icon={icon} title={title} isActive={!!isActive} />
                </Link>
              </li>
            );

            if (requiresAuth) {
              return <SignedIn key={idx}>{item}</SignedIn>;
            }
            return item;
          })}
          ...
          <SignedOut>
            <li
              ...
              className={pillClass('hover:w-[90px] sm:hover:w-[102px]')}
            >
              <Link href="/sign-in" aria-label="Entrar" className={surfaceClass}>
                <PillContents icon={<IoPersonOutline />} title="Entrar" />
              </Link>
            </li>
          </SignedOut>
          ...
          <li
            ...
            className={pillClass('hover:w-[78px] sm:hover:w-[84px]')}
          >
            <button
              type="button"
              onClick={toggleTheme}
